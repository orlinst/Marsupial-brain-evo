...
load("./Data/surface/data_surface.RData")
load("./Data/surface/surface_standartized_res_body.RData")
tree <-read.tree("./Data/tree176.nwk")

summary <-read.csv("./Data/surface/surface_summary.txt", sep = "\t", header = TRUE) 
...
#plotting the summary table of thetas

library(ggplot2)
library(reshape2)
require(wesanderson)
names(wes_palettes)
coco <- wes_palette("Cavalcanti1")
...
df1 <- data.frame(Body, Residuals, Regime)
df2 <- melt(df1, id.vars='Regime')
head(df2)

ggplot(df2, aes(x=Regime, y=value, fill=variable)) +
    geom_bar(stat='identity', position='dodge') + 
    scale_fill_manual(values = coco)+  coord_flip() +
