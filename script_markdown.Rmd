...
subsetOZNG <- data[data$Origin == "1" | data$Origin == "2"  ,]


treeOZ <- clean.data(subsetOZ, tree)
treeOZ <- treeOZ$tree

treeNG <- clean.data(subsetNG, tree)
treeNG <- treeNG$tree

treeAM <- clean.data(subsetAM, tree)
treeAM <- treeAM$tree

treeOZNG <- clean.data(subsetOZNG, tree)
treeOZNG <- treeOZNG$tree
...
data$Status[data$Status == 3] <- 2 # recode thylacine into 2
...
marsAM <- comparative.data(phy = tree, data = subsetAM, names.col = Names, 
                         vcv = TRUE, na.omit = FALSE, warn.dropped = TRUE)
marsOZNG <- comparative.data(phy = tree, data = subsetOZNG, names.col = Names, 
                         vcv = TRUE, na.omit = FALSE, warn.dropped = TRUE)
#####
...
model.pgls.res.OZNG <- pgls(log(Brain) ~ log(BodyN), data = marsOZNG, lambda='ML')
