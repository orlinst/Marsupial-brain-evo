...
```{r}
formula_dev <- Brain ~ BodyN + Litter.size*regime
formula_ori <- Brain ~ BodyN + Origin * regime
formula_vul <- Brain ~ BodyN + Status * regime
formula_tor <- Brain ~ BodyN + Torpor * regime
formula_fmr <- Brain ~ BodyN + FMR.Riek * regime
```
...
```{r}
names(solX)
names(solX) <- c("Intercept",    "Parental Care", "Mating System", "Body Size")
